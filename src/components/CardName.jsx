...
const CardName = ({ name, username, title, repositories, stars, darkMode, toggleTheme }) => {
  return (
    <div className={`relative max-w-md m-auto w-full rounded overflow-hidden shadow-lg ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'} m-4 transition duration-500`}>
      <div className="absolute top-0 right-0 p-4">
        <button onClick={toggleTheme} className="focus:outline-none">
          <FontAwesomeIcon icon={darkMode ? faSun : faMoon} size="lg" className={darkMode ? 'text-yellow-500' : 'text-gray-900'} />
        </button>
      </div>
      <div className="flex items-center p-4">
        ...
        <div>
          ...
          <div className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{username}</div>
        </div>
      </div>
      <div className="px-6 py-4">
        <div className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-2`}>{title}</div>
        <div className="flex items-center">
          <div className="mr-4">
            <span className={`inline-block ${darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-600'} rounded-full px-3 py-1 text-sm font-semibold mr-2`}>
            ...
            <span className={`inline-block ${darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-600'} rounded-full px-3 py-1 text-sm font-semibold`}>
